...
export async function saveReviewAction(_previousState: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!parsed.success) {
    return { ok: false, message: "Please check the highlighted fields. · চিহ্নিত তথ্যগুলো ঠিক করে দিন।", errors: parsed.error.flatten().fieldErrors };
  }
  ...
  try {
    ...
    if (!checkRateLimit(`review:${fingerprint}`)) {
      return { ok: false, message: "Too many attempts. Please try again in a few minutes. · অনেকবার চেষ্টা করা হয়েছে। কয়েক মিনিট পর আবার চেষ্টা করুন।" };
    }
    ...
    if (intent === "update") {
      if (!existingToken || !reviewId) {
        return { ok: false, message: "We could not verify ownership of this review. · এই রিভিউটির মালিকানা যাচাই করা যায়নি।" };
      }
      ...
      if (data) {
        revalidatePath("/");
        return { ok: true, message: "Your review has been updated. Thank you! · আপনার রিভিউ হালনাগাদ হয়েছে। ধন্যবাদ!" };
      }
      return { ok: false, message: "We could not verify ownership of this review. · এই রিভিউটির মালিকানা যাচাই করা যায়নি।" };
    }
    ...
    revalidatePath("/");
    return { ok: true, message: "Thank you for your feedback ❤️ · আপনার মূল্যবান মতামতের জন্য ধন্যবাদ ❤️" };
  } catch (error) {
    console.error("Review save failed", error);
    return { ok: false, message: "We could not save your review. Please try again. · আপনার রিভিউ সংরক্ষণ করা যায়নি। আবার চেষ্টা করুন।" };
  }
}
